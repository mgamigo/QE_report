...
FUNCTION efermig( et, nbnd, nks, nelec, wk, Degauss, Ngauss, is, isk )
  ...
  !! the Fermi energy
  REAL(DP) :: nelec_ef
  !! number of  electrons for the determined Fermi energy
  !
  ! ... local variables
  !
  ...
  if( Ngauss_ > 0  .or.  Ngauss_ == -1 ) then ! If methfessel-paxton method or Cold smearing method
    ...
    call newton_minimization(sq_num_electrons_minus_nelec, dev1_sq_num_electrons, dev2_sq_num_electrons, ef, eps, maxiter_aux, info)
  ...
  do i = 0, 1000
    Ef = -0.2d0 + (0.4d0/1000.d0)*i
    write(unit=9907, fmt="(4f30.16)") Ef, wgauss((efermig-Ef)/degauss,1), w0gauss((efermig-Ef)/degauss,1), w1gauss((efermig-Ef)/degauss,1)
  end do
  close(9907)
